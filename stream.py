...
with tabs[3]:
    ...
    st.session_state.gemini_key = st.text_input(
        ...
        type="password",
        help="Kept only in session memory. You can also set GOOGLE_API_KEY in environment.",
        key="gemini_key_ai_tab",
    )
    active_api_key = resolve_api_key()
    available_models = list_generate_models(active_api_key)
    default_models = ["gemini-1.5-flash", "gemini-2.0-flash", "gemini-1.0-pro"]
    model_options = available_models if available_models else default_models
    model_name = st.selectbox("Gemini model", options=model_options, index=0)
    if available_models:
        st.caption(f"Detected {len(available_models)} models with generateContent support.")
    else:
        st.caption("Could not fetch models list. Using safe fallback models.")
    c_test, c_gen = st.columns(2)
    if c_test.button("Test Gemini Connection", use_container_width=True):
        with st.spinner("Testing Gemini API connection..."):
            ok, result = get_gemini_response("Reply with: Gemini connection successful.", api_key=active_api_key, model_name=model_name)
        if ok:
            st.success("Gemini connected successfully.")
        else:
            st.error(result)
    if c_gen.button("Generate AI Insights", type="primary", use_container_width=True):
        with st.spinner("Generating AI insights..."):
            prompt = build_gemini_prompt(active_df, detected)
            ok, result = get_gemini_response(prompt, api_key=active_api_key, model_name=model_name)
        if ok:
